...
| **Stability score** | **84 / 100** | No crash-causing defects found anywhere. `dry_run`-by-default, atomic tmp-then-rename writes, and honest-degradation-over-fabrication are enforced consistently across every cluster (verified, not assumed). Deducted for: 1 real import-time cycle family (§2, corrected from an original overstated 2) and 2 outsized single-file hubs (`server.js` 288KB/~195 endpoints, `mission_control_api.py` 102KB) that concentrate blast radius. |
...
| **Technical debt score** | **72 / 100** | Real, named, mostly small items: one already-self-disclosed, now-lower-priority niche-scorer redundancy (`market_analyzer.py` — real but zero live UI callers, corrected §5); two independently-built, uncompared evidence-source catalogs (`evidence_network.py` vs. `multi_source_intelligence/`); one confirmed dead function (`multi_source_intelligence/aggregator.py::accumulate_evidence()`); 1 real import cycle family; and one real, disclosed-but-never-wired capability (`evidence_completeness.py`, corrected §2 — not a tooling bug, a genuine dormant module). None are correctness bugs. All were found via evidence, including two corrections made to this report's own first-pass findings during roadmap planning. |
...
> **Correction (2026-07-30, post-commit, before roadmap synthesis):** this section originally reported "10 real import cycles, 2 families" and "1 confirmed false positive" in the zero-dependents list. Both claims were re-verified independently at implementation-planning time and found overstated/wrong. Corrected below. The error mode in both cases: treating any textual/grep match (a function-local import, or a prose mention of a module's name in a comment) as equivalent to a real, trustworthy signal from `dependency_graph.py`'s own module-level-only cycle detector — which the tool's own docstring explicitly warns against. Left visible rather than silently edited, per this review's own "evidence only" standard.

**6 real import cycles, 1 family** (re-run via `dependency_graph.find_cycles()` with its actual documented default, `module_level_only=True` — the only reading the tool's own docstring calls trustworthy, since a deferred/function-body import "is a real, common way this codebase already breaks an otherwise-real cycle... counting it as a live edge would report a cycle Python itself would never hit"):

1. **Production plugin-registration cycle** (6 variants, 1 root cause, CONFIRMED real under both the strict and loose reading): `book_generator → inspectors → profit_oracle → product_families → product_families.families → {kdp_books | knowledge_bases | automation_systems → generic_adapter → {asset_generation.builders.pdf_builder | content_generation.generators.groq_generator | dossier_bundle.build_bundle}} → book_generator`. All module-level imports, verified via `dependency_graph.build_graph(module_level_only=True)['graph']` directly (not inferred). Root cause: 5 concrete plugin files (`product_families/families/kdp_books.py:10`, `knowledge_bases.py:17`, `asset_generation/builders/pdf_builder.py:19`, `content_generation/generators/groq_generator.py:20`, `dossier_bundle/build_bundle.py:18`) each do a module-level `import book_generator as bg` — the one edge that closes the loop back from the plugin tree to the generic dispatcher's own top-level import of `inspectors`/`profit_oracle`/`product_families`. This is the real, genuinely-real-today cycle — see the Roadmap for the specific fix.

2. **NOT real cycles, reclassified**: `executive_board ↔ value_engine ↔ factory_orchestrator` and `master_loop ↔ mission_control_api` **only appear when function-local (deferred) imports are counted as live edges** (`module_level_only=False` — the reading the tool's own docstring calls "noisier," never the default). Independently verified: `value_engine.py:475`'s `import factory_orchestrator as fo`, `factory_orchestrator.py:85`'s `import executive_board as eb`, `executive_board.py:279`'s `import value_engine`, `master_loop.py:169`'s `import mission_control_api`, and both `mission_control_api.py:1839`/`:1849`'s `import master_loop` are **all indented, function-body imports** — the exact deliberate escape hatch this codebase already uses everywhere (same idiom `dossier_bundle/build_bundle.py`'s own comment names: "a module-level import here would be circular"). These are real *logical* coupling (the modules do reference each other) but not real *import-time* cycles — Python never actually hits them. `master_loop.py` importing `mission_control_api.py` (even lazily) still technically reads against `infrastructure_bridge.py`'s documented preference that other modules not import `mission_control_api.py` as a library — worth a documentation note, not a P0/P1 refactor.

**28 modules flagged "zero dependents"** by the graph's own `find_zero_dependent_modules()`. The previously-reported "confirmed false positive" (`evidence_completeness.py`) was re-investigated and **the tool was actually correct** — independently verified via `grep -rn "^\s*import evidence_completeness\|^\s*from evidence_completeness import"` (zero matches anywhere outside its own tests) and `dependency_graph.dependents_of('evidence_completeness')` (returns `[]` under both readings). The 5 files originally cited as "real importers" (`autonomous_operations_status.py`, `decision_engine/engine.py`, `decision_engine/types.py`, `evidence_network.py`, `mission_control_api.py`) only mention the string "evidence_completeness" inside **comments/docstrings**, never an actual import statement. **The real finding this surfaces**: `evidence_completeness.py` (ADR-127, real, well-built, `__main__`-runnable) is genuinely dormant in production — `decision_engine/engine.py` accepts an optional `evidence_report` parameter it was designed to receive, but no caller anywhere in the codebase imports the module to compute one, so it is always `None` today. This is a real, disclosed-but-unwired capability, not a tooling bug — see Roadmap. Several other zero-dependent entries (`orchestrator/engines/*`, `market_intelligence_core/scoring/*`, `multi_source_intelligence/connectors/*`) remain confirmed-intentional (self-register via `pkgutil` at import time, invisible to a static import scan) — not re-verified as a batch, still an open item (Roadmap #2).
...
| Legacy Market Analyzer (market_analyzer.py) | DISCOVERY/placeholder, live endpoint, zero real UI callers (corrected, §5 item 4) | LOW (tech), LOW (trust — disclosure already present in API response, no UI surfaces it) | **KEEP** — no action needed unless a real UI caller is added; a prior session already declined the profit_oracle merge pending real usage data |
...
| Master Loop | REAL but narrow-in-use; its lazy import of `mission_control_api` is real coupling but NOT a real import-time cycle (corrected, §2) | LOW — documentation-level rule tension, not a live risk | **KEEP** — optionally document the exception, not urgent |
| Factory Orchestrator | REAL; its `value_engine`/`executive_board` cycle is real coupling but NOT a real import-time cycle (corrected, §2 — all 3 edges are function-local) | LOW | **KEEP** |
| Orchestrator Package (orchestrator/) | REAL/production, the actual live execution engine | LOW | **KEEP** |
| Dependency Graph tool | REAL, self-verified by use in this review; its zero-dependents output was re-verified CORRECT (§2) — no fix needed | LOW | **KEEP** |
...
1. **Break the 1 real import cycle** (production plugin-registration family, §2). Defer the 5 concrete plugin files' module-level `import book_generator as bg` (`product_families/families/kdp_books.py:10`, `knowledge_bases.py:17`, `asset_generation/builders/pdf_builder.py:19`, `content_generation/generators/groq_generator.py:20`, `dossier_bundle/build_bundle.py:18`) to function-local scope — the exact same safe idiom already proven elsewhere in this codebase. Closes 5 of 6 cycle variants; the 6th (`asset_generation.builders.pdf_builder → product_families`, via `from product_families.spec import components_to_sections`) needs one more deferred import in the same file. *Effort: low (6 one-line moves, zero logic change). Value: high — the one confirmed real coupling risk in the whole review.* See the separate implementation plan for full detail.

2. **Decide the fate of `evidence_completeness.py`** (ADR-127) — a real, well-built, disclosed evidence-completeness engine that is genuinely never invoked in production (confirmed: zero real imports anywhere outside its own tests; `decision_engine/engine.py`'s `evidence_report` parameter it was designed to receive is always `None` today). Either wire a real caller (a product decision, not a refactor — needs founder input on where in the pipeline it should compute) or document it as an intentionally-deferred capability the way `global_opportunity_exchange.py` already models. *Effort: low to document, medium to wire. Value: medium — closes a real, previously-mischaracterized gap.*

3. **Manually verify the remaining ~26 "zero-dependent" modules** flagged by `dependency_graph.py` (28 total; `evidence_completeness.py` re-verified as correctly flagged, not a false positive — see §2). Cheap per-module grep check; prevents both accidental future deletion of load-bearing code and continued uncertainty about real surface area. *Effort: low. Value: medium.*

4. **`market_analyzer.py` — lower priority than originally scored.** Re-verified: `POST /api/market-analyze` has **zero UI callers anywhere** (`grep` across `index.html`/`mission_control.html`/`mission_control_executive_v1.html` — no matches; the endpoint is reachable only via direct API call), and the API response **already includes the honest disclosure** (`"analysis_confidence": "static_reference_table"` + a plain-language `note` field, confirmed present in `analyze_market()`'s real return value). The "misleading the founder" risk this item was scored against does not exist today — nobody is looking at this data through the UI. Its own docstring also already explicitly declined a `profit_oracle.py` merge as "a larger, riskier change better suited to its own session with real usage data" (STRUCTURAL_DIAGNOSIS.md disease #2) — that prior decision stands; do not revisit without new usage data. *Effort: n/a — no action needed unless/until a real UI caller is added. Value: low today.*
...
**Post-commit correction note**: this report was committed once, then two of its own headline findings (§2 — the import-cycle count and the dependency-graph "false positive") were independently re-verified and corrected during roadmap planning, in place, with the original claims left visible rather than silently edited. Treat this as the current, corrected version.
