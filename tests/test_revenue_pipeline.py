...
class TestRunRevenuePipeline(unittest.TestCase):
    ...
    def tearDown(self):
        for p in (self.decisions_path, self.timeline_path, self.outcomes_path, self.analysis_db_path,
                  self.board_path, self.alerts_path, self.reopen_log_path):
            if os.path.exists(p):
                os.remove(p)
    ...
    def test_default_never_executes_real_production(self):
        ...
        result = pipeline.run_revenue_pipeline(
            ...
            board_path=self.board_path, alerts_path=self.alerts_path, reopen_log_path=self.reopen_log_path,
        )
    ...
    def test_reopen_history_is_isolated_and_honestly_empty_with_no_real_reopen(self):
        d = _accepted_decision()
        store.append_decision(d, path=self.decisions_path)
        result = pipeline.run_revenue_pipeline(
            decisions_path=self.decisions_path, outcomes_path=self.outcomes_path, timeline_path=self.timeline_path,
        ...
        self.assertEqual(result["results"][0]["reopen_history"], [])
        self.assertFalse(os.path.exists(self.reopen_log_path), "a read-only lookup must never create the reopen log")
    ...
    def test_execute_true_calls_the_real_orchestrator_with_execute_production_true(self, mock_run_cycle):
        ...
        pipeline.run_revenue_pipeline(
            ...
            timeline_path=self.timeline_path, analysis_db_file=self.analysis_db_path, board_path=self.board_path, alerts_path=self.alerts_path, reopen_log_path=self.reopen_log_path,
        )
