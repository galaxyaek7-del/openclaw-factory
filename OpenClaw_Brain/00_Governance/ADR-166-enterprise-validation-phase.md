...
- **Evidence integrity — a real, disclosed gap found by this round, with a real second-order cause**: `data/evidence_ledger.jsonl` (`evidence_engine.py`, ADR-163) **does not exist on disk at all**. Every one of the 10 named evidence types reports `verification_status: "NOT VERIFIED"`, `evidence_count: 0`. Investigating why turned up a genuine repo-hygiene bug, distinct from ADR-166's own "no new functionality" scope but disclosed and fixed here since it directly explains the finding: `reality_audit.py::audit_all_endpoints()`'s `record_evidence` parameter (the exact wiring ADR-163's own CLAUDE.md section claims exists) was **written to the working tree during the ADR-163 round but never actually committed** — `git show dac990a:reality_audit.py` (ADR-163's real commit) confirms the file at that commit has no `record_evidence` support at all; the working-tree version carrying it was silently uncommitted local drift ever since, 3 renumbered ADRs and 16+ commits later, never caught by any prior round's own "scoped git add" review because no prior round's `git status` happened to include `reality_audit.py`. Committed separately as its own small fix (not bundled into this ADR's own commit) once found. This explains, but does not excuse, why the evidence ledger stayed empty even through ADR-165's own "live-verified" claims — the wiring genuinely wasn't part of any shipped commit until this round.
