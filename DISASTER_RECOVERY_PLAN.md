...
| Configuration (`config/*.json`) | ✅ Yes | git-tracked (6 files: capability_registry, channels, economics, book_hivenotes_v1, jsonl_duplication_baseline, reality), pushed | ✅ full git history |
...
| **Automatic backups** | `recovery/snapshot.py`'s `snapshot_before()` generalizes the existing `.corrupt-{ts}.bak` quarantine pattern into a reusable helper — snapshots the 9 real state files in `DEFAULT_SNAPSHOT_TARGETS` (`data/factory_state.json`, `data/decisions.jsonl`, `data/market_evidence.jsonl`, `data/board_meetings.jsonl`, `data/paddle_products.json`, `data/commission_ledger.jsonl`, `data/affiliate_clicks.jsonl`, `data/safe_mode_state.json`, `data/publish_protection_state.json`) immediately before a real Paddle publish call and before an orchestrator production/publishing stage runs. Missing targets are skipped, never an error. Best-effort, never blocks the real operation. **Automatic Git commits were deliberately not built** — this conflicts with this session's own standing rule ("never commit unless explicitly asked") and remains gated behind the founder's separate, explicit go-ahead, exactly as flagged when this system was first architected. | `recovery/snapshot.py`, wired into `channels/paddle_arm.py` and `orchestrator/orchestrator.py`'s `_run_stage()` |
