...
def process_opportunity(decision, execute=False, timeline_path=None, decisions_path=None,
                         ...
                         board_path=None, alerts_path=None, reopen_log_path=None, evidence_path=None):
    ...
    # Executive Board Integration (2026-07-23): informational only, per
    # founder decision -- the board's verdict does not block this
    # function's own execute= safety switch (factory_orchestrator.py's
    # advisory_only=True already covers hard-gating when a caller wants
    # it at that layer). This is a read-only lookup, never a new meeting
    # convened from inside a revenue run.
    import executive_board as eb
    board_brief = eb.get_latest_board_brief(decision["niche"], board_path=board_path)

    # Market Evidence & Alerting layer (2026-07-23): same informational,
    # read-only connection as board_brief above -- never blocks execute=,
    # never triggers a new alert scan from inside a revenue run.
    ...
    # Value Engine (2026-07-23): same informational, read-only
    # connection -- Priority Score/Expected ROI/Strategic Value/cost
    # estimates/Recommendation for this niche. Reuses the cost result
    # already computed above (never a second estimate_production_cost()
    # call). Never blocks execute=, never itself decides resource
    # allocation -- that stays a human/Mission Control action reading
    # this real output.
    import value_engine
    value_profile = value_engine.compute_value_profile(
        decision["niche"], decisions_path=decisions_path, board_path=board_path,
