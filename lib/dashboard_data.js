// Galaxy Forge — Executive Dashboard data aggregator.
//
...
// Executive Board Integration (2026-07-23): reads the last real board
// meeting from data/board_meetings.jsonl (executive_board.py's own
// append-only log) — same pure-JS, no-Express/child_process-dependency
// pattern as readLatestMarketIntelligence() above, so this stays unit-
// testable in isolation (tests/test_dashboard_data.js) with no live
// server. Surfaces only a compact summary (not the full 10-executive
// meeting record, which dashboard.html has no use for) — real fields
// only, never recomputed.
function readLatestBoardMeetingSummary(logPath = path.join(FACTORY_DIR, 'data', 'board_meetings.jsonl')) {
  const entries = jsonl.readJsonlEntries(logPath, 1);
  if (!entries.length) {
    return { available: false, note: fs.existsSync(logPath) ? 'السجل فارغ' : 'لم يجتمع مجلس الإدارة التنفيذي بعد' };
  }
  const last = entries[0];
  const summary = last.decision_summary || {};
  return {
    available: true,
    niche: last.niche || null,
    decision_type: last.decision_type || null,
    board_decision: (last.tally || {}).board_decision || null,
    confidence: summary.confidence != null ? summary.confidence : null,
    risk_count: Array.isArray(summary.risks) ? summary.risks.length : 0,
    recommended_action_count: Array.isArray(summary.recommended_actions) ? summary.recommended_actions.length : 0,
    follow_up_task_count: Array.isArray(summary.follow_up_tasks) ? summary.follow_up_tasks.length : 0,
    convened_at: last.convened_at || null,
  };
}
...
function readFailedJobsSummary(logPath = path.join(FACTORY_DIR, 'factory_loop.log'), limit = 500) {
  ...
  if (!ticks.length) {
    return { failed_count: 0, recent: [], by_step: {}, note: fs.existsSync(logPath) ? 'السجل فارغ' : 'factory_loop.js لم يُشغَّل بعد' };
  }
  const failures = [];
  ...
  // Executive Intelligence Core, Round 3 (2026-07-29): real pattern
  // grouping on top of the same real failure list -- which step fails
  // most often, so a recurring problem is visible without reading every
  // individual entry by hand. No new data source, no invented severity.
  const byStep = {};
...
// Executive Intelligence Core, Round 3 (2026-07-29): real support-ticket
// pattern summary -- data/support_tickets.jsonl was write-only (server.js's
// POST /api/customer/support-ticket) with zero reader anywhere. Honestly
// empty today (zero real customer traffic through the Customer Platform
// yet) — never backfilled, same discipline as readCustomerReviewsSummary().
function readSupportTicketSummary(ticketsPath = path.join(FACTORY_DIR, 'data', 'support_tickets.jsonl')) {
  const tickets = readJsonlTail(ticketsPath, 1000);
  if (!tickets.length) {
    return { count: 0, open_count: 0, recent: [], note: 'No support tickets yet.' };
  }
  const openCount = tickets.filter(t => (t.status || 'OPEN') === 'OPEN').length;
  const recent = tickets.slice(-10).reverse().map(t => ({
    ticket_id: t.ticket_id, submitted_at: t.submitted_at, status: t.status || 'OPEN',
    related_request_id: t.related_request_id || null, message: (t.message || '').slice(0, 200),
  }));
  return { count: tickets.length, open_count: openCount, recent };
}
...
// Engineering Evolution Mode follow-up: this used to be its own
// implementation — now a thin wrapper over the shared
// lib/jsonl.js#readJsonlEntries(), the canonical version this file's own
// copy became the template for. Kept as a local name (not replaced at
// every one of this file's 6 internal call sites) so this diff stays
// small and every call site keeps reading the same as before.
function readJsonlTail(filePath, limit) {
  return jsonl.readJsonlEntries(filePath, limit);
}

// Real Customer Accounts (Customer Platform Round 6, 2026-07-29) -- real
// signed-up customers (data/customer_accounts.json, written by lib/
// customer_auth.js-backed signup route). Password hashes never leave this
// function -- only name/email/company/created_at, the same public-facing
// account shape server.js's own publicAccountView() already exposes.
function readCustomerAccountsSummary(accountsPath = path.join(FACTORY_DIR, 'data', 'customer_accounts.json')) {
  const data = readJsonSafe(accountsPath);
  if (!data || typeof data !== 'object') {
    return { count: 0, recent: [], note: 'No customer accounts yet.' };
  }
  ...
  const recent = accounts
    ...
    .slice(0, 10)
    .map(a => ({ name: a.name, email: a.email, company: a.company || '', created_at: a.created_at }));
  return { count: accounts.length, recent };
}

// Real Customer Reviews (Customer Platform Round 5, 2026-07-29) --
// genuinely greenfield until a real review lands: data/customer_reviews.jsonl
// only ever gains a line via customer_pipeline.py's submit_review(), gated
// on a real DELIVERED/FOLLOWED_UP order. Zero reviews is an honest,
// reportable state, never backfilled with a placeholder testimonial --
// same discipline as every other "real zero" this dashboard already
// reports (e.g. tier1_discovery.accepted).
function readCustomerReviewsSummary(reviewsPath = path.join(FACTORY_DIR, 'data', 'customer_reviews.jsonl')) {
  const reviews = readJsonlTail(reviewsPath, 1000);
  if (!reviews.length) {
    return { count: 0, average_rating: null, recent: [], note: 'No customer reviews yet.' };
  }
  ...
  const recent = reviews.slice(-10).reverse().map(r => ({
    rating: r.rating, text: r.text || '', submitted_at: r.submitted_at,
  }));
  return {
...
function computeDashboard({ health = null, awareness = null, priorities = null } = {}) {
  ...
  return {
    ...
    latest_board_meeting: readLatestBoardMeetingSummary(),
  };
}
...
module.exports = {
  ...
  readCustomerReviewsSummary,
  readCustomerAccountsSummary,
  readSupportTicketSummary,
};
