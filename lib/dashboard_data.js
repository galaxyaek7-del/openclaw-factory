...
function readAttentionFlag(filePath = path.join(FACTORY_DIR, 'NEEDS_ATTENTION.md')) {
  return { active: fs.existsSync(filePath) };
}
...
// Composes every real signal above into one view. `health` and `awareness`
// are passed in (computed by server.js's own computeHealthStatus()/
// self_awareness.assessSelfAwareness() — both already exist and are reused,
// not reimplemented) so this module stays free of server.js's Express/
// child_process dependencies and fully unit-testable on its own.
function computeDashboard({ health = null, awareness = null } = {}) {
  return {
    ...
    needs_attention: readAttentionFlag(),
