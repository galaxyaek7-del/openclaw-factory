...
function clearCurrentTask(statePath = DEFAULT_STATE_PATH) {
  try {
    ...
    // Autonomous Company Runtime (ADR-157, 2026-07-31): same real,
    // pre-existing bug fixed in factory_state.py's Python mirror --
    // active_workflow was set by setCurrentTask() but never cleared
    // here, staying permanently "sticky" and making the already-shipped
    // recovery-status panel show a stale active workflow forever.
